...
pub(crate) async fn execute<'e, DB, E, Q>(executor: E, query: Q) -> Result<u64, sqlx::Error>
where
    DB: Backend,
...
pub(crate) async fn fetch_all<'e, DB, E, O, Q>(executor: E, query: Q) -> Result<Vec<O>, sqlx::Error>
where
    DB: Backend,
    E: Executor<'e, Database = DB>,
...
pub(crate) async fn fetch_one<'e, DB, E, O, Q>(executor: E, query: Q) -> Result<O, sqlx::Error>
where
    DB: Backend,
    E: Executor<'e, Database = DB>,
    O: for<'r> FromRow<'r, DB::Row> + Send + Unpin,
...
pub(crate) async fn fetch_optional<'e, DB, E, O, Q>(
    executor: E,
    query: Q,
