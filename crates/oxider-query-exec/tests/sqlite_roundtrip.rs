...
//!
//! Note there is no `.render(&Sqlite)` at any call site: the exec functions take
//! the query directly and render it with the pool's dialect.
...
async fn insert_then_select_roundtrips() {
    ...
    let rows: Vec<User> = fetch_all(&pool, User::query()).await.unwrap();
    assert_eq!(
        rows,
        vec![User {
            id: 1,
...
async fn update_and_delete_affect_rows() {
    ...
    let updated = execute(
        &pool,
        User::update().set(User::age, 31).filter(User::id.eq(1)),
    )
    .await
    .unwrap();
    assert_eq!(updated, 1);

    let one: User = fetch_one(&pool, User::query().filter(User::id.eq(1)))
        .await
        .unwrap();
    assert_eq!(one.age, 31);

    let deleted = execute(&pool, User::delete()).await.unwrap();
    assert_eq!(deleted, 1);

    let gone: Option<User> = fetch_optional(&pool, User::query().filter(User::id.eq(1)))
        .await
        .unwrap();
    assert!(gone.is_none());
}
